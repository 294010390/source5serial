 ...
 *
 *  Rewritten for multi-touch screen device:
 *  Copyright (C) 2013 Liang Bailin.
 *
...
struct ts_sample {
	...
	struct timeval	tv;
	unsigned char	mt_num;		/* The number of touch point at the same time */
	unsigned char	mt_id;		/* This point is which one in these same time points. 0 ~ (mt_num - 1) */
};

#define TS_MT_POINT_MAX_NUM	3	/* The max number of touch point at the same time */
